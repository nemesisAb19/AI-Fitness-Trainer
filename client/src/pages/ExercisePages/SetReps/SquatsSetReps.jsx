...
const SquatsSetReps = () => {
  ...
  const startExercise = async () => {
    try {
      const response = await axios.post("http://localhost:5001/api/start-squats-exercise", {
        sets,
        reps,
        rest_time: parseInt(restTime) || 30,
      });
      console.log(response.data.message);
      alert("Squats exercise started!");
      setShowModal(true);
    } catch (error) {
  ...
  useEffect(() => {
    const handleStorage = (e) => {
      if (e.key === "exerciseDone" && e.newValue === "true") {
        alert("Squats exercise completed!");
        localStorage.removeItem("exerciseDone");
        stopRecording();
        setShowModal(false);
        setLoading(false);
        setVideoError(false);
        navigate("/exercise-complete/squats");
      }
    };
  ...
  const captureScreenshot = () => {
    ...
    const a = document.createElement("a");
    a.href = image;
    a.download = `squats_screenshot_${Date.now()}.png`;
    a.click();
  };

  const toggleFullscreen = () => {
    const el = document.querySelector(".squats-video-modal");
  ...
  return (
    <>
      <Navbar />
      <div className="squats-setup-page">
        <motion.div
      ...
      {showModal && (
        <div className="squats-video-modal">
          <div className="squats-video-header">
            ...
            <div className="squats-video-controls">
          ...
          {!videoError && (
            <img
              src="http://localhost:5001/squat_feed"
              alt="Exercise Stream"
