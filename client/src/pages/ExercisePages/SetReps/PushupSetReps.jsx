...
const PushupSetReps = () => {
  ...
  const startExercise = async () => {
    try {
      const response = await axios.post("http://localhost:5001/api/start-pushups-exercise", {
        sets,
        reps,
        rest_time: parseInt(restTime) || 30,
      });
      console.log(response.data.message);
      alert("Push Ups exercise started!");
      setShowModal(true);
    } catch (error) {
  ...
  useEffect(() => {
    const handleStorage = (e) => {
      if (e.key === "exerciseDone" && e.newValue === "true") {
        alert("Push Ups exercise completed!");
        localStorage.removeItem("exerciseDone");
        stopRecording();
        setShowModal(false);
        setLoading(false);
        setVideoError(false);
        navigate("/exercise-complete/pushup");
      }
    };
  ...
  const captureScreenshot = () => {
    ...
    const a = document.createElement("a");
    a.href = image;
    a.download = `pushups_screenshot_${Date.now()}.png`;
    a.click();
  };

  const toggleFullscreen = () => {
    const el = document.querySelector(".pushups-video-modal");
  ...
  return (
    <>
      <Navbar />
      <div className="pushups-setup-page">
        <motion.div
      ...
      {showModal && (
        <div className="pushups-video-modal">
          <div className="pushups-video-header">
            ...
            <div className="pushups-video-controls">
          ...
          {!videoError && (
            <img
              src="http://localhost:5001/push_feed"
              alt="Exercise Stream"
